...
const App = () => {
  ...
  function data1() {
    try {
      ...
      const hueCenter1 = parseInt(inputs[0].value) ?? 0;
      const hueRange1 = parseInt(inputs[1].value) ?? 0;
      const saturationCenter1 = parseInt(inputs[2].value) ?? 0;
      const saturationRange1 = parseInt(inputs[3].value) ?? 0;
      const valueCenter1 = parseInt(inputs[4].value) ?? 0;
      const valueRange1 = parseInt(inputs[5].value) ?? 0;
      const hueCenter2 = parseInt(inputs[6].value) ?? 0;
      const hueRange2 = parseInt(inputs[7].value) ?? 0;
      const saturationCenter2 = parseInt(inputs[8].value) ?? 0;
      const saturationRange2 = parseInt(inputs[9].value) ?? 0;
      const valueCenter2 = parseInt(inputs[10].value) ?? 0;
      const valueRange2 = parseInt(inputs[11].value) ?? 0;
      const areas = document.getElementsByClassName("area");
      const area1 = parseInt(areas[0].value) ?? 0;
      const area2 = parseInt(areas[1].value) ?? 0;
